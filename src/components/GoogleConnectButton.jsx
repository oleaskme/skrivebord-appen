import { useEffect, useState } from 'react'
import { useUser } from '../context/UserContext'
import { supabase } from '../lib/supabase'

export default function GoogleConnectButton() {
  const { activeUser, loadUsers } = useUser()
  const [email, setEmail] = useState(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    // Sjekk om brukeren allerede har koblet Google
    supabase
      .from('users')
      .select('google_account_email')
      .eq('id', activeUser.id)
      .single()
      .then(({ data }) => {
        setEmail(data?.google_account_email ?? null)
        setLoading(false)
      })
  }, [activeUser.id])

  useEffect(() => {
    // Vis melding etter redirect fra OAuth-callback
    const params = new URLSearchParams(window.location.search)
    if (params.get('googleConnected') === 'true') {
      loadUsers()
      supabase
        .from('users')
        .select('google_account_email')
        .eq('id', activeUser.id)
        .single()
        .then(({ data }) => setEmail(data?.google_account_email ?? null))
      window.history.replaceState({}, '', '/')
    }
  }, [activeUser.id, loadUsers])

  if (loading) return null

  if (email) {
    return (
      <div className="flex items-center gap-2 text-sm text-green-600 bg-green-50 border border-green-200 rounded-lg px-3 py-1.5">
        <span className="text-green-500">✓</span>
        <span className="font-medium">{email}</span>
      </div>
    )
  }

  return (
    <a
      href={`/api/auth/google?userId=${activeUser.id}`}
      className="flex items-center gap-2 text-sm text-white bg-primary-500 hover:bg-primary-600 rounded-lg px-3 py-1.5 transition-colors"
    >
      <GoogleIcon />
      Koble til Google
    </a>
  )
}

function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#fff" opacity=".9"/>
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#fff" opacity=".9"/>
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#fff" opacity=".9"/>
      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#fff" opacity=".9"/>
    </svg>
  )
}
