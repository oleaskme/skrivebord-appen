...
export default function NewMasterModal({ folderId, inputDocs = [], onClose, onCreated, onAIResult }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      // 1. Opprett master-dokumentet
      const { data: doc, error: err } = await supabase
        .from('master_documents')
        .insert({ folder_id: folderId, name: name.trim(), ai_instruction: aiInstruction.trim() || null, content: '' })
        .select().single()
      if (err) throw err

      onCreated(doc)

      // 2. Kjør AI hvis input-dokumenter er valgt
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={isRunning ? undefined : onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]"
        ...
        {isRunning ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-4 py-16 px-8">
            ...
            <p className="text-sm font-medium text-gray-600">Kjører AI-analyse av {selectedInputIds.length} INPUT-dokument{selectedInputIds.length !== 1 ? 'er' : ''}...</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
            <div className="flex-1 overflow-y-auto px-8 pb-4 space-y-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Beskriv dokumentets formål og hva AI skal se etter i INPUT-dokumentene..."
              ...
              {inputDocs.length > 0 && (
                <div>
                  ...
                  <p className="text-xs text-gray-400 mb-2">Velg hvilke INPUT-dokumenter AI skal lese for å fylle ut det nye dokumentet.</p>
                  <div className="border border-gray-200 rounded-lg overflow-hidden divide-y divide-gray-100 max-h-52 overflow-y-auto">
                    {inputDocs.map(doc => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={selectedInputIds.includes(doc.id)}
                  ...
                  {selectedInputIds.length > 0 && (
                    <p className="text-xs text-primary-600 font-medium mt-1.5">
                      {selectedInputIds.length} dokument{selectedInputIds.length !== 1 ? 'er' : ''} valgt — AI kjøres automatisk etter opprettelse
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="flex gap-3 px-8 py-5 border-t border-gray-100 shrink-0">
              ...
              >
                {selectedInputIds.length > 0 ? `Opprett og kjør AI (${selectedInputIds.length} dok.)` : 'Opprett'}
              </button>
